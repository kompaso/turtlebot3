...
class TurtleTrajectoryGenerator:
    ...
    def __init__(self):
        ...
        self.trajectory_pub = rospy.Publisher('/turtle/trajectory', CartesianTrajectory, queue_size=-1)
...
if __name__ == '__main__':
    rospy.init_node('turtle_trajectory_generator_node')
    turtle_tg = None
    while not rospy.is_shutdown():
        try:
            turtle_tg = turtleTrajectoryGenerator()
            turtle_tg.spin()
        except rospy.ROSInterruptException as e:
            turtle_tg.close()
            del turtle_tg
            print('Restart turtle_trajectory_generator_node with new ros-sim-time')
